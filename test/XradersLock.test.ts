...
describe("XradersLock", function () {
  ...
  describe("Deployment", function () {
    it("Should set the correct unlockPeriod and penaltyRate", async function () {
      const { xradersLock, unlockPeriod, penaltyRate } = await loadFixture(
  ...
  describe("Locking Tokens", function () {
    ...
    it("Should fail to lock fractional tokens", async function () {
      ...
      await expect(
        xradersLock.connect(addr1).lock(fractionalAmount, deadline, v, r, s)
    ...
    it("Should fail to lock zero tokens", async function () {
      ...
      await expect(
        xradersLock.connect(addr1).lock(zeroAmount, deadline, v, r, s)
  ...
  describe("Unlocking Tokens", function () {
    it("Should unlock tokens", async function () {
      ...
      expect(unlocks[0].timestamp).to.be.gte(
        (await time.latest()) + unlockPeriod - 1
      );
    });
    ...
    it("Should not unlock zero tokens", async function () {
      ...
      await expect(
        xradersLock.connect(addr1).unlock(hre.ethers.parseEther("0"))
    ...
    it("Should not unlock more tokens than locked", async function () {
      ...
      await expect(
        xradersLock.connect(addr1).unlock(hre.ethers.parseEther("200"))
      ).to.be.revertedWith("Invalid unlock amount");
    });
  });
  ...
  describe("Redeeming Tokens", function () {
    ...
    it("Should revert redeem with no unlocks", async function () {
      const { xradersLock, addr1 } = await loadFixture(
        deployXradersLockFixture
      );

      await expect(xradersLock.connect(addr1).redeem()).to.be.revertedWith(
        "No unlock found"
      );
    });
    ...
    it("Should revert fast redeem with no unlocks", async function () {
      ...
      await expect(xradersLock.connect(addr1).fastRedeem(0)).to.be.revertedWith(
    ...
    it("Should revert fast redeem with invalid index", async function () {
      ...
      await expect(xradersLock.connect(addr1).fastRedeem(1)).to.be.revertedWith(
        "Invalid unlock index"
      );
    });
    ...
    describe("Owner-Only Functions", function () {
      ...
      it("Should revert when non-owner tries to set unlock period", async function () {
        ...
        await expect(
          xradersLock.connect(addr1).setUnlockPeriod(10 * 24 * 60 * 60)
        )
